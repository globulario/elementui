...
class DropdownMenuElement extends PolymerElement {
    ...
    ready() {
        ...
        // Set the default font/icon color
        ...
        // I will get the list of menu items from the html element remove from it
        var items = []

        // Set parent items in items.
    ...
    appendItem(item) {
        ...
        if (item.level == 0) {
            ...
            currentPanel.element.style = "display: flex; background-color: " + this.background + ";"
            // I will move child into dropdown_submenu div.
            var childs = []
            ...
            item.content = currentPanel.appendElement({ "tag": "div" }).down()
                .appendElement({ "tag": "div", "id": item.id, "class": "menu_item_div", "style": "display: block; position: relative; z-index: 1; padding: 2px 4px 2px 4px; margin: 1px; min-width: 300px; transition: all .2s ease; position: relative; z-index: 1; padding: 2px 4px 2px 4px; margin: 1px; min-width: 300px; transition: all .2s ease; color: " + this.color + ";" }).down()

            // separate items from top.
        ...
        dropdown_submenu_items_style += "box-shadow: 0px 1px 12px -1px rgba(0, 0, 0, 0.75);"


        // Append the subitem panel.
        item.panel = currentPanel.appendElement({ "tag": "div", "class": "dropdown_submenu_items", "style": dropdown_submenu_items_style }).down()
        ...
        // Now the actions...
        currentPanel.element.onclick = function (item) {
            return function (evt) {
                evt.stopPropagation()
        ...
        item.content.element.onmouseover = function (dropdownMenu, item) {
            return function (evt) {
                ...
                // In case the separator is define I will not set the background color.
