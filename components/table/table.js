...
import ResizeObserver from "resize-observer-polyfill/dist/ResizeObserver.es.js"; // List of imported functionality.

import { createElement } from "../element.js";
import { randomUUID, rgbToHsl, randomIntFromInterval, CountSpace, fireResize, isString, intersectSafe, exportToCsv } from "../utility.js";
...
class TableElement extends PolymerElement {
  ...
  createTiles() {
    ...
    var resizeListener = function (tiles, scrollDiv, header, menu, table) {
      return function (entry) {
        ...
        if (!menu.hidemenu) {
          // set the menu.
          menu.element.parentNode.style.marginLeft = menu.element.children[0].offsetWidth - 1 + "px";
          menu.element.style.left = -1 * menu.element.offsetWidth - 1 + "px";
        }
      };
  ...
  ready() {
    ...
    if (!this.hidemenu) {
      ...
      this.appendChild(this.menu.element); // Postion the menu.
